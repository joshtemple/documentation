...
* The return value of the function should be an array of [entities](/docs/understanding-tracking-design/understanding-events-entities/index.md) to be added to the event
...
* Avoid including CPU-intensive tasks in your enrichment code (e.g. encryption) without thoroughly benchmarking the impact they might have on your event processing time.
...
function process(event) {
  ...
  if (entities) {
    ...
    for (const entity of entities.data) {
      if (entity.schema === 'iglu:org.my-company/my-schema/jsonschema/1-0-0') {
...
The return value of the `process` function is an array of extra entities to add to the event. So adding entities is as simple as returning them!
